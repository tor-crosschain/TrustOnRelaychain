...
def start_gateways_from_para_to_relay_local(config_name: str, chain_configs: List[dict], tag):
    ...
    for chain_id, config in enumerate(chain_configs):
        ...
        relay_identifier = f"relay_tor_{timestamp}_{chain_id}_{relay_chain_id}.log"
        cmd = f"nohup python -u relayer/tor/gateway_from_para_to_relay.py --srcid {chain_id} --dstid {relay_chain_id} --header_private_key '{header_key}' --tx_private_key '{tx_key}' --config_name '{config_name}' --tag '{tag}' >{logpath}/{relay_identifier} 2>&1 &"
        cmds.append(cmd)
    for cmd in cmds:
        out = exec_command_local(cmd=cmd, cmd_type="start tor relayer from parachain to relay", close_fds=True)
        print(cmd)
        print(out, out == '\r\n')
...
def start_gateways_from_relay_to_para_local(config_name: str, chain_configs: List[dict], tag):
    ...
    for chain_id, config in enumerate(chain_configs):
        ...
        relay_identifier = f"relay_tor_{timestamp}_{relay_chain_id}_{chain_id}.log"
        cmd = f"nohup python -u relayer/tor/gateway_from_relay_to_para.py --srcid {relay_chain_id} --dstid {chain_id} --header_private_key '{header_key}' --tx_private_key '{tx_key}' --config_name '{config_name}' --tag '{tag}' >{logpath}/{relay_identifier} 2>&1 &"
        cmds.append(cmd)
    for cmd in cmds:
        out = exec_command_local(cmd=cmd, cmd_type="start tor relayer from relay to parachain", close_fds=True)
        print(cmd)
        print(out, out == '\r\n')
  

def start_gateways_from_para_to_para_local(config_name: str, chain_configs: List[dict], tag):
    relay_chain_id = 0
    
    # transfer to new account on parachains
    # two dimensions
    # (1, 2)
    keys_para = [[0]*len(chain_configs) for i in range(len(chain_configs))]
    def create(chain_id, config, keys_para):
        transferHashes = []
        node = config['server_nodes'][0]
        url = f"http://{node['host']}:{node['rpcport']}"
        ethsdk = EthSdk(url=url, poa=True, unlock_genesis=False)
        for src_chain_id, src_config in enumerate(chain_configs):
            if src_chain_id == relay_chain_id: continue
            ...
            tx_key = account.key.hex()        
            address = account.address
            txhash1 = ethsdk.sendSingleTransaction(
                addr_from=ethsdk.defaultAccount,
                addr_to=address,
                value=ethsdk.toWei(1),
                wait=False
            ...
            tx_key = tx_account.key.hex()
            tx_address = tx_account.address
            txhash2 = ethsdk.sendSingleTransaction(
                addr_from=ethsdk.defaultAccount,
                addr_to=tx_address,
                value=ethsdk.toWei(1),
                wait=False
            )
            transferHashes.append(txhash1)
            transferHashes.append(txhash2)
            print(f"transfer to tx account({tx_address}) ok!")

            keys_para[src_chain_id][chain_id] = (header_key, tx_key)
    ...
    for src_chain_id, src_config in enumerate(chain_configs):
        if src_chain_id == relay_chain_id: continue
        for dst_chain_id, dst_config in enumerate(chain_configs):
            if dst_chain_id == relay_chain_id: continue
            if dst_chain_id == src_chain_id: continue
            header_key, tx_key = keys_para[src_chain_id][dst_chain_id]
            relay_identifier = f"relay_tor_{timestamp}_{src_chain_id}_{dst_chain_id}.log"
            cmd = f"nohup python -u relayer/tor/gateway_from_para_to_para.py --srcid {src_chain_id} --dstid {dst_chain_id} --header_private_key '{header_key}' --tx_private_key '{tx_key}' --config_name '{config_name}' --tag '{tag}' >{logpath}/{relay_identifier} 2>&1 &"
            cmds.append(cmd)
    for cmd in cmds:
        out = exec_command_local(cmd=cmd, cmd_type="start tor relayer from parachain to parachain", close_fds=True)
        print(cmd)
        print(out, out == '\r\n')
    

def send_cm_local(config_name: str, chain_configs: List[dict], each_num: int):
    relay_chain_id = 0
    
    def send_cm_to_each_chain(chain_id, config):
        logger.add(f"scheduler_log_nor/send_cm_chain_{chain_id}.log")
...
def wait_finish(chain_configs, target: int):
    ...
    logger.add(f"scheduler_log_tor/wait_finish.log")
...
def statistical_info(config_name, chain_configs, tag):
    info_root_path = f"info/tor/{config_name}/{tag}"
    info_parachain_path = f"info/tor/{config_name}/{tag}/parachain"
    info_relaychain_path = f"info/tor/{config_name}/{tag}/relaychain"

    clean_data = {
        'crossSendToR': [], # from user
        'SubmitHeader': [], # from relay to parachain
        'crossReceiveFromParaToR': [], # from  para to para
        'SubmitHeaderAndBCR': [], # from para to relay
    }
    for i in range(1, len(chain_configs)):
        for j in range(1, len(chain_configs)):
            if i == j: continue
            filename = os.path.join(info_parachain_path, f'{i}-{j}-info.ignore.json')
    ...
    for i in range(1, len(chain_configs)):
        filename = os.path.join(info_relaychain_path, f'{i}-{0}-info.ignore.json')
        if not os.path.exists(filename): continue
        infos: List[Dict[str, int]] = json.load(fp=open(filename, 'r'))
        for info in infos:
            key = list(info.keys())[0]
...
def main():
    ...
    start_gateways_from_para_to_para_local(config_name, chain_configs, tag=str(each_num))
    start = time.time()

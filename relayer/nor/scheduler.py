...
def start_gateways_from_para_to_para_local(config_name: str, chain_configs: List[dict], tag):
    ...
    def create(chain_id, config, keys_para):
        ...
        for src_chain_id, src_config in enumerate(chain_configs):
            if src_chain_id == abandoned_chain_id: continue
    ...
    for chain_id, config in enumerate(chain_configs):
        if chain_id == abandoned_chain_id: continue
    ...
    for src_chain_id, src_config in enumerate(chain_configs):
        ...
        for dst_chain_id, dst_config in enumerate(chain_configs):
            if dst_chain_id == abandoned_chain_id: continue
            if dst_chain_id == src_chain_id: continue
            header_key, tx_key = keys_para[src_chain_id][dst_chain_id]
            relay_identifier = f"relay_nor_{timestamp}_{src_chain_id}_{dst_chain_id}.log"
            cmd = f"nohup python -u relayer/nor/gateway_from_para_to_para.py --srcid {src_chain_id} --dstid {dst_chain_id} --header_private_key '{header_key}' --tx_private_key '{tx_key}' --config_name '{config_name}' --tag '{tag}' >{logpath}/{relay_identifier} 2>&1 &"
            cmds.append(cmd)
    for cmd in cmds:
        out = exec_command_local(cmd=cmd, cmd_type="start nor relayer from parachain to parachain", close_fds=True)
        print(cmd)
        print(out, out == '\r\n')
...
def statistical_info(config_name, chain_configs, tag):
    info_root_path = f"info/nor/{config_name}/{tag}"
    info_parachain_path = f"info/nor/{config_name}/{tag}/parachain"
    
    clean_data = {
        'crossSend': [], # from user
        'SubmitHeader': [], # from relay to parachain
        'crossReceiveFromPara': [], # from  para to para
    }

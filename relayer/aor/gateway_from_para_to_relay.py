...
class GatewayFromParaToRelay:
    ...
    def send_header(self, block: BlockData):
        ...
        assert receipt.status == 1, f"submit header({block['number']}) failed! txhash: {receipt['transactionHash'].hex()}"
        print(f"submit block({block['number']}) of chain-{self.chain_id} to chain-{self.dst_chain_id} ok!")
        infokey = f"{self.chain_id}-SubmitHeaderToRelay-{receipt['transactionHash'].hex()}"
    ...
    def deal_txs(self, txhashes):
        for txhash in txhashes:
            ...
            if not str(func.fn_name).startswith("crossSend"):
                ...
                continue
            # # not check dst_chain_id if the dst is relay chain
            ...
            print(f"obtain cross-chain message index on parachain-{self.chain_id}: {cmIndex}")
    ...
    def query_construct_dst_tx(self, height: int, cmIndex: int) -> bytes:
        ...
        print(f"tx_nonce: {self.tx_nonce}")

        # txhash, _ = self.dst_ethsdk.contract_transact(
        #     contract_address=self.dst_address, 
        #     contract_abi=self.dst_abi, 
        #     exec_func="crossReceiveOnRelay", 
        #     func_args=[
        #         cm, 
        #         cmHash, 
        #         position, 
        ...
        print(f"submit cross-chain message({cmIndex}) in block({height}) of para chain-{self.chain_id} to relay chain-{self.dst_chain_id} ok! txhash: {txhash.hex()}")
    ...
    def deal_txhash(self):
        while True:
            ...
            if status != 1:
                ...
                decoded_input = contract.decode_function_input(transaction["input"])
                func, params = decoded_input[0], decoded_input[1]
                print(f"cross-chain message transaction on relay-chain-{self.dst_chain_id} fails! func: {func.fn_name}; params: {params}; to: {transaction['to']}; txhash: {txhash.hex()}")
            else:
                infokey = f"{self.chain_id}-crossReceiveOnRelay-{txhash.hex()}"
                self.collect_info(Info(key=infokey, value=receipt['gasUsed'])) 
                print(f"cross-chain message transaction on relay-chain-{self.dst_chain_id}-(height={receipt['blockNumber']}) succeeds! txhash: {txhash.hex()}")

def collect_info(info_queue: multiprocessing.Queue, config_name: str, srcid, dstid, tag):
    info_path = f"info/aor/{config_name}/{tag}/allchains"

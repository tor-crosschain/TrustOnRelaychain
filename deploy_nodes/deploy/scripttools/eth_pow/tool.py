...
class Tool(object):
    ...
    def generate_genesis(self, genesis_account, **kwargs):
    ...
    def pre_start(self, chain_config: dict, **kwargs):
        """
        start bootnode
        """
    ...
    def start_chain(self, chain_config: dict, **kwargs):
        ...
        """
        使用 genesis_account 创建 genesis config
        """
        genesis_config = self.generate_genesis(genesis_account['address'])
        account_files = [(genesis_account['key_filename'], genesis_account['key_filecontent'])]
    ...
    def create_valid_account(self, chain_config: dict, idx_node: int, password: str, **kwargs) -> dict:
        ...
        rpc = 'http://{}:{}'.format(node['host'], node['rpcport'])        
        sdk = EthSdk(rpc,poa=False, unlock_genesis=True)
        # 这里 import 之后，在节点的keystore中生成密钥文件，密钥文件的文件名体现了“时间”，但是这个时间是节点所在服务器的时间，而不是执行 create_account_from_raw 的服务器的时间
        # 两个服务器之间会有时间偏差； 
        ...
        receipt = sdk.sendSingleTransaction(addr_from=sdk.defaultAccount, addr_to=new_account['address'], value=sdk.toWei(1000))
    ...
    def reg_listen(self, chain_config: dict):
        ...
        sdk = EthSdk(gover_rpc,poa=False)
    ...
    def deploy_gover_contract(self, chain_config, **kwargs):
        ...
        result = goverctr_manager.deploy_all(
            ...
            ispoa=ispoa, 
            account=account, 
            solc_path='./blockchain/bin/solc_cc', 
    ...
    def update_crossctr(self, chain_config: dict, ctr_info: tuple):
        ...
        gover_sdk = EthSdk(gover_rpc, poa=False)
    ...
    def buildzone_confirm(self, gover_config: dict, inter_chain_config: dict, **kwargs):
        ...
        # logger.info("crossinfo length: {}".format(len(inter_crossinfo)))
    ...
    def update_zone_rpc(self, inter_config: dict, gover_config: dict):
        ...
        sdk = EthSdk(main_rpc_addr, poa=False)

...
def start(idx: str, chain_config: dict):
    ...
    **kwargs: 
    ...
    geth_poa = os.path.join(setting.BIN_DIR, 'geth_1.10.0')
    piddir = os.path.join(setting.CACHE_PID, idx)
    child = os.path.join(setting.ROOT_DIR, idx)
    os.makedirs(child)
    ...
    # 以太坊需要初始化区块链
    chain_init(geth_poa, datadir, logdir_init, genesis)
    # 启动区块链
    other_args = "--rpcapi 'personal,eth,net,web3,miner,txpool,admin,clique' --allow-insecure-unlock"
    pid, rpcport = chain_start(geth_poa, datadir, logdir_start, bootnode, other_args)
